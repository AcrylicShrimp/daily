...
class DocumentStorage:
    def __init__(self, top_k_search: int = 20, top_k_rerank: int = 5):
        ...
        self.embeddings = VoyageAIEmbeddings(model="voyage-3-lite", batch_size=64)
    ...
    def query(self, query: str) -> list[Document]:
        searched = self.vector_store.search(query, "similarity", k=self.top_k_search)
    ...
    async def aquery(self, query: str) -> list[Document]:
        searched = await self.vector_store.asearch(
            query, "similarity", k=self.top_k_search
        )
